...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Contactez Teranga Bridge Africa à Dakar pour vos projets d'approvisionnement agroalimentaire." },
      { property: "og:title", content: "Contact — Teranga Bridge Africa" },
      { property: "og:description", content: "Sacré Cœur 3 VDN, Dakar, Sénégal · +221 33 892 07 21" },
    ],
  }),
...
function ContactPage() {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setErrors({});
    const subject = encodeURIComponent(`Demande de contact — ${result.data.name}`);
  ...
  return (
    <section className="container-page py-16 md:py-24">
      <div className="max-w-3xl">
        ...
        <h1 className="mt-3 text-4xl md:text-5xl font-bold text-foreground">Parlons de votre projet</h1>
        <p className="mt-5 text-lg text-muted-foreground">Notre équipe vous répond sous 24h ouvrées.</p>
      </div>

      <div className="mt-12 grid gap-10 lg:grid-cols-5">
        <div className="lg:col-span-2 space-y-5">
          <InfoCard icon={MapPin} title="Adresse">Sacré Cœur 3 VDN, 57<br />Dakar, Sénégal</InfoCard>
          <InfoCard icon={Phone} title="Téléphone"><a href="tel:+221338920721" className="hover:text-primary">+221 33 892 07 21</a></InfoCard>
          <InfoCard icon={Mail} title="Email"><a href="mailto:contact@terangabridgeafrica.com" className="hover:text-primary break-all">contact@terangabridgeafrica.com</a></InfoCard>

          <div className="overflow-hidden rounded-2xl border border-border shadow-soft">
            <iframe
              title="Localisation Teranga Bridge Africa"
        ...
        <form onSubmit={onSubmit} className="lg:col-span-3 rounded-2xl border border-border bg-card p-7 md:p-9 shadow-soft space-y-5">
          ...
          {status === "ok" && <p className="text-sm text-primary">Merci ! Votre client mail s'ouvre…</p>}
        </form>
      </div>
    </section>
  );
